...
# Personalize the pattern by setting the stitch rate (i.e., number of stitches you knit per minute)
...
pat <- dft %>% 
  ...
  mutate(pattern = str_replace_all(pattern, "_", ", mb, k"),
...
p = df %>% 
  ggplot(aes(x = x, y = y)) +
  #geom_jitter(pch = 15, size = 0.5, stroke = 1, width = 3, height = 0, seed = 1984)
  geom_point(pch = 15, size = 0.5, stroke = 1)
if (method == "circular") {
...
p = p + scale_x_reverse(breaks = seq(from = nstitches, to = 1, by = -1)) +
  ...
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_line(size = 0.5, colour= "darkgray"),
...
```{r, fig.alt="Knitting chart illustrating the approximate location of the bobbles on a 200-stitch by 40-row knitting pattern."}
